...
class MobileController extends BaseController
{
    ...
    protected function buildTableParames($excludeFields = [])
    {
        ...
        if(iset_field($this->model,'add_id')){
        ...
        foreach ($filters as $key => $val) {
            ...
            if ($this->relationSearch && count(explode('.', $key)) == 1) {
                $key = "{$tableName}.{$key}";
            }else if($this->relationSearch && count(explode('.', $key)) == 2){
                $key = explode('.', $key)[0].".".explode('.', $key)[1];
            }else if($this->relationSearch && count(explode('.', $key)) == 3){
                $key = CommonTool::humpToLine(explode('.', $key)[0]).".".CommonTool::humpToLine(explode('.', $key)[1]).".".explode('.', $key)[2];
            }
    ...
    private function checkAuth(){
        ...
        // 验证登录
        if (!in_array($currentController, $mobileConfig['no_login_controller']) &&
            !in_array($currentNode, $mobileConfig['no_login_node'])) {
                session('currentNode', request()->url());
                if(!session('openid')){
                    if(is_weixin()){//微信浏览器
                        $config = [
                ...
                empty($adminId) && $this->redirect( __url('login/index'));
                // 判断是否登录过期
