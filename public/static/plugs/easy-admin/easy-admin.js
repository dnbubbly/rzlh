    ...
    var admin = {
        ...
        table: {
                ...
                $.each(data, function (i, v) {
                    if (v === 'refresh') {
                        ...
                        $.each(v, function (ii, vv) {
                            ...
                            vv.auth = vv.auth || '';
                            vv.url = vv.url || '';
                            vv.width = vv.width || '400';
                            vv.height = vv.height || '300';
                    ...
                    if (d.field !== false && d.search !== false) {
                        switch (d.search) {
                            ...
                            case  'selectTree':
                                ...
                                $.each(d.selectList, function (sI, sV) {
                                    var selected = '';
                                    if (sV[0] === d.searchValue) {
                                        selected = 'selected=""';
                                    }
                                    selectHtml += '<option value="' + sV[0] + '" ' + selected + '>' + sV[1] + '</option>/n';
                                });
                                formHtml += '\t<div class="layui-form-item layui-inline">\n' +
                                    '<label class="layui-form-label">' + d.title + '</label>\n' +
                                    '<div class="layui-input-inline">\n' +
            ...
            buildToolbarHtml: function (toolbar, tableId) {
                ...
                toolbar.width = toolbar.width || '400';
                toolbar.height = toolbar.height || '300';
                ...
                formatToolbar.width = formatToolbar.width !== '' ? 'data-width="' + formatToolbar.width + 'px"' : '';
                formatToolbar.height = formatToolbar.height !== '' ? 'data-height="' + formatToolbar.height + 'px"' : '';
                formatToolbar.checkbox = toolbar.checkbox ? ' data-checkbox="true" ' : '';
                formatToolbar.tableId = tableId !== undefined ? ' data-table="' + tableId + '" ' : '';
                html = '<button ' + formatToolbar.class + formatToolbar.method + formatToolbar.extend + formatToolbar.checkbox +  formatToolbar.tableId + formatToolbar.width + formatToolbar.height + '>' + formatToolbar.icon + formatToolbar.text + '</button>';
        ...
        listen: function (preposeCallback, ok, no, ex) {
            ...
            $('body').on('click', '[data-table-export]', function () {
                ...
                var index = admin.msg.confirm('根据查询进行导出，确定导出？', function () {
                	url=table.where!==undefined?(url+"?filter="+table.where.filter+"&op="+table.where.op):url;
